...
$msgs = array();
$client_message_id = 0;
$discuss_id = !empty($_POST["did"]) ? intval($_POST["did"]) : 0;
$msgobj_h =& xoops_getmodulehandler("message");
if ($msgobj_h->checkDiscuss($discuss_id)) {
	$client_message_id = !empty($_POST["mid"]) ? intval($_POST["mid"]) : 0;
	if ($msgobj_h->processPostRequest()) {
		$msgs =& $msgobj_h->getMessages();
/* cache		
	} else {
		$msgs = array(array('message_id'=>$client_message_id+1, 'message'=>$msgobj_h->getErrors(), 'uname'=>'system', 'color'=>'00000'));
*/
	}
}
